...
/**
 * A single, self-contained comment component with its own editing state and actions.
 */
...
Comment.propTypes = {
  ...
  onReply: PropTypes.func.isRequired, // Added prop for replying
};


/**
 * The main component for the entire comments and activity section.
 */
function CommentsSection({ issueComments, currentUser, canDeleteAnyComment }) {
  ...
  const [newCommentFiles, setNewCommentFiles] = useState([]); // State for new comment attachments
  const [isSubmitting, setIsSubmitting] = useState(false);
  const commentInputRef = useRef(null); // Ref to focus the input field
  ...
  const handleAddComment = async () => {
    ...
    const newCommentObject = {
      ...
      text: newComment,
      attachments: newCommentFiles, // Add files to the new comment
      createdAt: new Date().toISOString(),
    };
    setComments([...comments, newCommentObject]);
    setNewComment('');
    setNewCommentFiles([]); // Clear files after submitting
  ...
  const handleReply = (authorName) => {
    ...
    commentInputRef.current.focus(); // Focus the text field
  };
      ...
      <Box sx={{ display: 'flex', gap: 2, mb: 3 }}>
        ...
        <Box sx={{ flexGrow: 1 }}>
          <TextField
            ...
            inputRef={commentInputRef} // Assign ref
            sx={{
